...
function App() {
  ...
  useEffect(()=>{
    if (funBtn) {
      setCurrentZArray(prev => prev = zArray.filter(object =>  object.tags.includes('fun')))
    } else if (casualBtn) {
      setCurrentZArray(prev => prev = zArray.filter(object =>  object.tags.includes('casual')))
    } else if (congressBtn) {
      setCurrentZArray(prev => prev = zArray.filter(object =>  object.tags.includes('congress')))
    } else if (smileBtn) {
      setCurrentZArray(prev => prev = zArray.filter(object =>  object.tags.includes('casual')))
    } else if (randomBtn) {
   ...
   //Set up the array to store the history of images with an object with two images diferent from each other.
    const [indexHistory, setIndexHistory] = useState(()=>{
      ...
      do {zObjLeftIndex = Math.floor(Math.random() * zArray.length)
          zObjRightIndex = Math.floor(Math.random() * zArray.length)}
      while (zObjLeftIndex === zObjRightIndex) 
    
    return [{zuckObjLeft:zArray[zObjLeftIndex], zuckObjRight:zArray[zObjRightIndex]}]
   ...
   function randomIndexs() {
    ...
    if (zArrayLeft.id !== zArrayRight.id &&
